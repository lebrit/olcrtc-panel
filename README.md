...
- Добавить проверку `Jitsi + vp8channel` на `SSRC is already used` и автоматически предлагать `datachannel` или другой Jitsi/WBStream.
- Добавить явную кнопку `Sub все профили` у пользователя, чтобы не путать её с профильной кнопкой `Sub`.
